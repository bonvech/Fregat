/**
 * \file nograph.cpp
 * \brief Работа с буфером fadc
 * 
 * Функции, выполняемые во время измерений. Часть класса fadc_board
 */
/// address of RG1 register
#define RG1ADDR 10
/// address of RG0 register
#define RG0ADDR 8

/// Strob begin
#define STROBB 0         //150
/// Strob end
#define STROBE 512       //350

public:

/** ----------------------------------------------------------------
 * get event
 * Read events from buffer and write it to binary data file (fout)
 * \param flag - write (0) or not (not 0) to file *fkadf= 'Event'
 * ---------------------------------------------------------------*/
//unsigned int get_event(void)
        ...
        for (i = 0; i < Buf2; i++)
        {
            for(j = 0; j < CHANMAX; j++)
            {
                ...
                if(i < 6)
                {
                    ...
                    printf( " %3i", fadc_data);  // print to screen
                    if(dout) fprintf(dout, " %3i", fadc_data);  // print to screen
                }
#endif
            }
            if(!flag)
...
// --------------------------------------------------------------
/// returns 1 if there is error in RG0: bit 8
