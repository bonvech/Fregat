...
#ifndef _FADC_BOARD
#define _FADC_BOARD
/// output file write fadc information
#define WORK_FILE "./log/fadc_work.dat"
#define DELAY 100
...
{
public:
    unsigned int   AddrOn[16], SerNum[16];
    ssize_t        BIN_size;
    unsigned short Buf2;
    unsigned short RG1put;

    fadc_board() // constructor
    {
        //short kk;
        BIN_size = 0;
        Buf2 = Work.buf2;
...
private:
    ...
    int boot_fadc(unsigned int BasAddr, char* buffer)
    {
        ...
        {	
            ...
            kk  = 1;
            rez = 0;
            if(! (num%10000) ) printf("\b\b\b\b\b\b\b%7i", num); 
            fflush(stdout);
            for(ii=0; ii < 8; ii++)
            {
                rez = b & kk;
	...
	printf("\a");
        ...
        if (ii != 2)  return 0; // error 
        return 1;  // OK
    }


    //---------------------------------------------------------
    // returns number of inituated fadc boards
...
public:
    //---------------------------------------------------------
    //int read_fadc_temp(unsigned int addr)
...
//---------------------------------------------------------
//int read_fadc_temp(unsigned int addr)
// 2008.02.09  pri obrashenii k RX16 v rezhime  raboty kanalov na priem
//poluchaet bredovye znachenija temperatury,
//pri initsializatsii rabotaet pravilno
/*
float read_average_fadc_temp(unsigned char addrr)
{
    unsigned int Rez1 = 0, Summ = 0;
    float Temp1 = 0.;
    unsigned short iiz = 0, numbr = 0;
    
    for(iiz = 1; iiz <= AddrOn[0]; iiz++, numbr++)
    {
        BaseAddr = AddrOn[iiz];
        usleep(6000);
        printf("BaseAddr = %xh", BaseAddr);
        if( !RX16(addrr, &Rez1) )
        {
            ...
            Summ += Rez1;
            printf("Rez = %i Summ = %i num = %i\n", Rez1, Summ, numbr);
            if(dout) fprintf(dout,"Rez = %i Summ = %i num = %i\n", Rez1, Summ, numbr);
            }
            
            if(numbr)   Summ /= numbr;
            else       return -100.0; // error
                
                printf("Rez = %i Summ = %i num = %i\n", Rez1, Summ, numbr);
            if(dout) fprintf(dout,"Rez = %i Summ = %i num = %i\n", Rez1, Summ, numbr);
            
            //printf(" Summ = %i; ", Summ);
            Temp1 = kod_2_fadc_temp(Summ);
            printf("  Sum = %i T1av = %6.2f oC\n", Summ, Temp1);
            if(dout) fprintf(dout, "  Sum = %i T1av = %6.2f oC\n", Summ,  Temp1);
            return Temp1;
            }
            */
